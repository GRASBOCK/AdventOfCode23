...
fn total_heatloss_from_to(origin: &(i32, i32), target: &(i32, i32), blocks: &Grid<u8>) -> usize {
    let path_tiles: Box<[Path]> = vec![
        Path {
            heatloss_since_origin: usize::MAX,
            from_dir: (0, 0)
        };
...
fn solve_part1(input: &PuzzleInput) -> usize {
    total_heatloss_from_to(
        &(0, 0),
        &(input.width as i32 - 1, input.height as i32 - 1),
        input,
    )
}
...
4322674655533";
    ...
    macro_rules! example_parsed {
        () => {
            PuzzleInput {
                tiles: Box::new([
                    2, 4, 1, 3, 4, 3, 2, 3, 1, 1, 3, 2, 3, 3, 2, 1, 5, 4, 5, 3, 5, 3, 5, 6, 2, 3,
                    3, 2, 5, 5, 2, 4, 5, 6, 5, 4, 2, 5, 4, 3, 4, 4, 6, 5, 8, 5, 8, 4, 5, 4, 5, 2,
                    4, 5, 4, 6, 6, 5, 7, 8, 6, 7, 5, 3, 6, 1, 4, 3, 8, 5, 9, 8, 7, 9, 8, 4, 5, 4,
                    4, 4, 5, 7, 8, 7, 6, 9, 8, 7, 7, 6, 6, 3, 6, 3, 7, 8, 7, 7, 9, 7, 9, 6, 5, 3,
                    4, 6, 5, 4, 9, 6, 7, 9, 8, 6, 8, 8, 7, 4, 5, 6, 4, 6, 7, 9, 9, 8, 6, 4, 5, 3,
                    1, 2, 2, 4, 6, 8, 6, 8, 6, 5, 5, 6, 3, 2, 5, 4, 6, 5, 4, 8, 8, 8, 7, 7, 3, 5,
